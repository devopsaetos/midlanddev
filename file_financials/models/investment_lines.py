...
class InvestmentLineExt(models.Model):
    ...
    def _onchange_predefine_plan_id(self):
        # Picking a plan on the line IS what makes it "own_plan" - no separate
        # checkbox for the user to remember to tick.
    ...
    def calculate_amount_and_values(self):
        ...
        # Gate on predefine_plan_id alone, not own_plan - own_plan is only set
        # by the _onchange_predefine_plan_id above, so records whose plan was
        # set some other way (import, copy, or before that onchange existed)
