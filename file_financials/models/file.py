...
class FileExtension(models.Model):
            ...
            for pre_plan in self.predefine_plan_id.predefine_plan_line_ids:
                ...
                if self.env.ref('real_estate.down_payment_product').id == pre_plan.product_id.id:
                    self.down_payment_amount = round(self.sale_amount * (
                            pre_plan.value / 100) if pre_plan.basis == 'percentage' else pre_plan.value)
                    self.down_payment_interval = pre_plan.interval
                    self.down_payment_frequency = pre_plan.frequency
                    self.down_payment_start = pre_plan.start_from
    ...
    def create_installment_plan(self):
        ...
        if all([self.starting_date, self.interval_id, self.total_installment, self.net_sale_amount]) and self.active:
            ...
            while True:
                if i > total_dates:
                    if not _pending_plan_events() or extra_dates > 120:
                        break
                    # prepare_next_iterator_value can leave `dates` empty, so fall
                    # back to the last scheduled line when extending the schedule
                    if dates:
                        base_date = dates[-1]
                    else:
                        plan_dates = self.installment_plan_ids.mapped('date')
                        base_date = max(plan_dates) if plan_dates else fields.Date.from_string(self.starting_date)
                    dates.append(base_date + relativedelta(months=+self.interval_id.nom))
                    total_dates = len(dates) - 1
                    extra_dates += 1
                # for rec in dates:
                ...
                if (self.down_payment_amount and self.type == 'normal' and not start_down_payment
                    ...
                    if installment_number == down_payment_position:
                        self.installment_plan_ids.create({
                            ...
                            'installment_number': installment_number,
                            'amount': self.down_payment_amount,
                            'tax_amount': round((self.down_payment_amount * tax_id[0].amount) / 100,
                                                2) if tax_id else 0,
                            'residual': self.down_payment_amount + round(
                                (self.down_payment_amount * tax_id[0].amount) / 100,
                                2) if tax_id else self.down_payment_amount,
                            'payment_status': 'not_paid',
                            'file_id': self.id
                        })
                ...
                if self.balloon_payment_frequency and not start_balloon_payment:
                    if installment_number == balloon_start:
                        if balance:
                            ...
                            amount = 0
                        self.installment_plan_ids.create({
                            # with include_installment the balloon shares the date of the
                            # installment that follows it (same month, separate lines)
                            'date': dates[i],
                            'installment_number': installment_number,
                        ...
                        balloon_interval += balloon_start + (1 if self.include_installment else 0)
                        start_balloon_payment = True
                        installment_number = installment_number + 1
                        if is_balloon_included:
                            # ensure a date remains after the shift so the paired
                            # installment lands on the balloon's date
                            if i >= total_dates:
                                dates.append(dates[-1] + relativedelta(months=+self.interval_id.nom))
                                total_dates = len(dates) - 1
