...
class AccountAccount(models.Model):
	...
	account_type = fields.Selection(selection_add=[('view', 'View')], ondelete={'view': 'cascade'}, default='view')
	...
	def _search(self, domain, offset=0, limit=None, order=None, **kwargs):
		...
		if not context.get('show_parent_account', False):
			new_domain = Domain([('account_type', '!=', 'view')]) & Domain(new_domain)
		return super(AccountAccount, self)._search(new_domain, offset=offset, limit=limit, order=order, **kwargs)

	@api.model
	def default_get(self, fields):
		res = super(AccountAccount, self).default_get(fields)
		if 'account_type' in fields:
			res['account_type'] = 'view'
		return res
