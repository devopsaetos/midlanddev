...
export class DNGridStack {
    ...
    constructor(orm){
        this.orm = orm
        this.itemsBasicData = new WeakMap() // to be used for storing items basic data , also used as key for gridstack elements
        ...
        this.gsConfigMap = {
            ...
            'is_new_dn': DEFAULT_CHART_CONFIG_CB,
        }
    }
    ...
    getConfig(itemBasicData, grid){
        ...
        const isNewDN = itemBasicData.is_new_dn
        let config;
        if(isNewDN){
            config = this.gsConfigMap['is_new_dn']?.(grid[id], id)
        }
        else{
            config = this.gsConfigMap[type]?.(grid[id], id) ?? this.defaultGSConfig
        }
