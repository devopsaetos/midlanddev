...
from odoo.addons.ks_dashboard_ninja.common_lib.ks_utils import am_format_read_group_data, DEFAULT_PRECISION_DIGITS, \
    DEFAULT_PAGINATION, MIN_INTEGRAL_DIGITS, get_allowed_integral_digits, get_allowed_precision, get_allowed_pagination
from odoo.addons.ks_dashboard_ninja.common_lib.ks_utils import replace_company_domain, replace_new_id
from odoo.addons.ks_dashboard_ninja.models.ks_item_source import GRANULARITY_SELECTION, ITEM_TYPE_METADATA, \
    ITEM_FRONTEND_DATA_STRUCTURE
from odoo.exceptions import ValidationError, UserError
from odoo.fields import Domain
from odoo.tools import sql, SQL, float_repr
...
class KsDashboardNinjaItems(models.Model):
    ...
    is_new_dn = fields.Boolean(string="Advanced Charts", default=False)
    type = fields.Selection(
        selection=lambda self: [(t, ITEM_TYPE_METADATA[t]['name']) for t in ITEM_TYPE_METADATA],
        default='card', required=True, help="Select the type of chart"
    )
    item_source_ids = fields.One2many('ks.item.source', 'item_id', string="Item Source")
    is_horizontal = fields.Boolean()
    
    #Formula Feature Fields
    formula_item_source_ids = fields.One2many('ks.item.source', 'formula_item_source_id')

    # Display Fields
    ks_pagination = fields.Integer(string='Pagination', default=DEFAULT_PAGINATION)
    renderer_oversized_behaviour = fields.Selection(selection=[
        ('wrap', "Wrap"), ('none', "None"), ('truncate', "Truncate"), ('hide', "Hide"), ('fit', "Fit"),
        ('wrap-no-break', "Wrap No Break")
    ], default='truncate')
    is_legend = fields.Boolean(string="Show Chart Legend", default=True, help="Display legend for chart series")
    card_layout = fields.Selection(selection=[('layout_1', 'Layout 1')], string="Card Layouts", default='layout_1')

    # Decimal Precision or Rounding
    # todo: float_rounding can be implemented as defined in method float_round
    enable_number_formatting = fields.Boolean()
    precision_digits = fields.Integer(default=DEFAULT_PRECISION_DIGITS)
    min_integral_digits = fields.Integer(default=MIN_INTEGRAL_DIGITS)
    ks_number_format_id = fields.Many2one('ks.number.format')

    _ks_pagination_positive = models.Constraint(
        'check(ks_pagination > 0)',
        'Pagination limit must be greater than 0.',
    )

    _decimal_precision_digits_range = models.Constraint(
        'check(precision_digits >= 0 and precision_digits <= 10)',
        'Precision digits must be between 0 and 10.',
    )

    _integral_digits_range = models.Constraint(
        'check(min_integral_digits > 0 and min_integral_digits <= 5)',
        'Integral Digits must be between 1 and 5.',
    )

    @api.constrains('item_source_ids')
    def _check_item_source_ids_max_limit(self):
        """Validate that item_source_ids does not exceed 10 records."""
        for record in self:
            if len(record.item_source_ids) > 10:
                raise ValidationError(_('Maximum 10 item sources are allowed per dashboard item.'))

    # Item Data Fields
    trigger_frontend_data_compute = fields.Integer()
    # Field used in case if we need to delete stored frontend data once
    # Maybe be useful when we are changing frontend structure
    # just change the name for the field it will delete frontend data once
    clear_frontend_data_1 = fields.Boolean(default=True)

    frontend_data = fields.Json(compute="compute_frontend_data", compute_sudo=False, store=True)
    # todo: later: make the data like frontend_data more robust by making the defined structure with defined key-value types
    # todo: check usage for source_data
    source_data = fields.Json(help='Cached source data to prevent unnecessary recomputation during preview')

    #todo: divide the display fields and donot compute on change of these fields
    @api.depends(
        'trigger_frontend_data_compute', 'item_source_ids', 'type', 'enable_number_formatting',
        'precision_digits', 'min_integral_digits', 'ks_number_format_id', 'is_legend', 'renderer_oversized_behaviour',
        'ks_pagination', 'is_horizontal', 'name', 'is_new_dn', 'formula_item_source_ids'
    )
    def compute_frontend_data(self):
        for record in self:
            if record.is_new_dn:
                result = record.prepare_frontend_data()
                record.frontend_data = json.loads(json.dumps(result, default=json_default))
                if record.clear_frontend_data_1:
                    record.clear_frontend_data_1 = False
            else:
                record.frontend_data = ITEM_FRONTEND_DATA_STRUCTURE

    def prepare_number_formatting_data(self):
        number_formatting_data = {}
        if self.enable_number_formatting:
            number_formatting_data.update({
                'number_format_lines': self.prepare_number_format_lines(),
                'enable_number_formatting': self.enable_number_formatting,
                'precision_digits': get_allowed_precision(self.precision_digits),
                'min_integral_digits': get_allowed_integral_digits(self.min_integral_digits)
            })

        return number_formatting_data


    def prepare_frontend_data(self, **kwargs) -> dict:
        '''
        Prepares the data required for frontend rendering of the dashboard item.
        :param kwargs:
        :return: list of dict
            'sources': [],
            'formula_sources': [],
            'errors': [],
            'formatted_groupby_record_mapped_data': default(dict),
            'groupby_data_values': [],
            'model_data': {},
        '''
        self.ensure_one()
        ...
        errors = []
        groupby_values = []
        mapped_groupby_formula_ctx = defaultdict(dict)
        formatted_groupby_record_mapped_data = defaultdict(dict)
        number_formatting_data = self.prepare_number_formatting_data()

        if kwargs.get('drill_down_data'):
            drill_down_data = kwargs['drill_down_data']
            current_sources_data = drill_down_data.get('current_sources_data', {})
            source_ids = [data['id'] for data in current_sources_data.values() if data.get('id')]
            item_source_ids = self.env['ks.item.source'].browse(source_ids).sorted('handle_sequence')
        else:
            item_source_ids = self.item_source_ids

        drill_down_data = kwargs.pop('drill_down_data', {'parent_sources_data': {}, 'current_sources_data': {}})

        for source_id in item_source_ids:
            if source_id.type != self.type:
                errors.append({
                    'sequence': source_id.sequence, 'name': source_id.name,
                    'error_data': {'msg': "Item Types Match Failed", 'item_source_name': source_id.name}
                })
                continue

            if source_id.item_id:
                drill_down_data['parent_sources_data'].update({
                    f'{source_id.sequence}': {
                        'model_data_sources': source_id.drill_down_item_source_ids.prepare_drill_down_model_data(),
                        'list_view_id': getattr(source_id.list_view_id, 'xml_id', False),
                        'form_view_id': getattr(source_id.form_view_id, 'xml_id', False),
                    }
                })
                drill_down_data['current_sources_data'].update({ f'{source_id.sequence}': {'domain': []} })

            frontend_data = source_id.prepare_frontend_data(
                pagination=get_allowed_pagination(self.ks_pagination), drill_down_data=drill_down_data,
                number_formatting_data=number_formatting_data, mapped_groupby_formula_ctx=mapped_groupby_formula_ctx,
                formatted_groupby_record_mapped_data=formatted_groupby_record_mapped_data,
                **kwargs
            )

            if frontend_data.get('error_data'):
                frontend_data['error_data']['item_source_name'] = source_id.name
                errors.append({
                    'sequence': source_id.sequence, 'name': source_id.name,
                    'error_data': frontend_data['error_data']
                })
                continue

            groupby_values.extend(frontend_data.get('groupby_values', []))
            source_data = {k: v for k, v in frontend_data.items() if
                           k not in ['source_data', 'formula_ctx', 'mapped_groupby_formula_ctx']}
            source_data.update({'sequence': source_id.sequence})
            sources.append(source_data)

        formula_sources = self.prepare_formula_sources(
            number_formatting_data, mapped_groupby_formula_ctx, formatted_groupby_record_mapped_data
        )
        ...
        return {
            ...
            'groupby_data_values': [{'__groupby_value': value} for value in dict.fromkeys(groupby_values)],
            'model_data': self.prepare_model_data(),
        }
    
    def prepare_formula_sources(
            self, number_formatting_data, mapped_groupby_formula_ctx, formatted_groupby_record_mapped_data
    ) -> list[dict]:
        metadata = ITEM_TYPE_METADATA.get(self.type, {})

        if not metadata or not metadata.get('is_formula_supported'):
            return []

        formula_sources = []
        formulas = self.formula_item_source_ids.filtered(
            lambda x: x.apply_on == 'aggregate_result' and x.type == self.type
        ).read(
            ['name', 'formula', 'apply_on', 'xy_type', 'is_bullets', 'is_area_chart', 'sequence']
        )

        for formula in formulas:
            replace_new_id(formula)
            sequence = formula['sequence']
            formula_data = {'model_data': formula, 'sequenced_aggregates': [f'{sequence}.__evaluated_value']}
            for groupby_value, formula_ctx in mapped_groupby_formula_ctx.items():
                group_record_data = {'__groupby_value': groupby_value}
                try:
                    evaluated_value = safe_eval(formula['formula'], formula_ctx)
                    evaluated_value = abs(evaluated_value)
                    if number_formatting_data.get('enable_number_formatting'):
                        evaluated_value = float_repr(evaluated_value, number_formatting_data['precision_digits'])

                    group_record_data[f'{sequence}.__evaluated_value'] = evaluated_value
                except Exception:
                    pass

                formatted_groupby_record_mapped_data[groupby_value].update(group_record_data)

            formula_sources.append(formula_data)

        return formula_sources
        

    def prepare_model_fields(self):
        fields = ['type', 'name']
        if self.type == 'xy_chart':
            fields += ['is_horizontal', 'renderer_oversized_behaviour', 'is_legend']
        elif self.type == 'grouped_list':
            fields += ['ks_pagination']

        return fields

    def prepare_model_data(self):
        fields = self.prepare_model_fields()
        data = self.read(fields)
        data = data[0]
        replace_new_id(data)
        data.update({'number_formatting_data': self.prepare_number_formatting_data()})
        return data

    def prepare_number_format_lines(self):
        if not (self.ks_number_format_id or self.ks_number_format_id.number_format_line_ids):
            return []
        return self.ks_number_format_id.number_format_line_ids.sorted('power_of_ten').mapped(
            lambda line: {'number': 10 ** line.power_of_ten, 'suffix': line.suffix}
        )
    ...
    @api.onchange('ks_dashboard_item_type', 'is_new_dn')
    def onchange_data_source_to_odoo(self):
        for record in self:
            if record.ks_dashboard_item_type == 'ks_scatter_chart':
                record.ks_data_calculation_type = 'custom'
            if record.is_new_dn:
                record.ks_data_calculation_type = 'custom'
                record.data_source = 'odoo'
    ...
    def get_item_config(self, params = {}, **kwargs):
        """
        :rtype : object
        :param : item object
        ...
        if self.is_new_dn:
            return self.prepare_frontend_data(**kwargs)
    ...
    def ks_sort_sub_group_by_records(self, ks_data, field_type, ks_chart_date_groupby, ks_sort_by_order,
        ...
        if ks_data:
            ...
            def _ks_mixed_x_sort_key(item):
                # `x` is used as the label key in chart data; it can be False/None for "Undefined"
                # groups, or a string/number/date depending on the grouped field.
                x = item.get('x')
                if x is None:
                    return (5, '')
                ...
                if isinstance(x, date):
                    return (2, datetime.combine(x, datetime.min.time()))
                try:
                    return (3, str(x).casefold())
                except Exception:
                    return (4, repr(x))
            ...
            for data in ks_data:
                if field_type == 'date_type':
                    ...
                    data['value'].sort(key=_ks_mixed_x_sort_key, reverse=reverse)
