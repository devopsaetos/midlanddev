...
class KsCustomReport(models.Model):
    ...
    ks_cr_top_menu_id = fields.Many2one('ir.ui.menu', default=lambda self: self.env.ref('ks_custom_report.menu_root'),
                                        string="Show Under Menu", required=True, domain=lambda self: self._get_dynamic_domain())
    ...
    def _get_dynamic_domain(self):
        default_menu = self.env.ref('ks_custom_report.menu_root', raise_if_not_found=False)
        mymenu = []
        if default_menu:
            mymenu = [default_menu.id]
        menus = self.env['ir.ui.menu'].search([('action', '=', False)])
        mymenu += menus.ids
        domain = []
        if default_menu:
            domain.append(('id', 'in', mymenu))
        return domain
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('ks_cr_query_type') == 'custom_query':
                try:
                    query_split = values.get('ks_custom_query').split()
                    model_name = query_split[[word.lower() for word in query_split].index('from') + 1]
                    ...
                    raise ValidationError(_("An error occurred while creating the view: %s") % str(e))

        recs = super(KsCustomReport, self).create(vals_list)
        for rec in recs:
            rec.sudo()._ks_create_db_table()
            rec.sudo()._ks_create_custom_report()
        return recs
    ...
    def write(self, vals):
        ...
        for res in self:
            ...
            if 'ks_cr_group_access' in vals:
                menu_update_vals['group_ids'] = vals['ks_cr_group_access']
            ...
            if rec.ks_cr_action_id:
                ...
                try:
                    ...
                    raise ValidationError(_("Pivot/Graph are not available in list"))
    ...
    def _ks_create_custom_report(self):
        # It Creates: Menu and Access Rights of the model
        self._ks_cr_menu_action(self)
        self._ks_cr_access_rights(self)
    ...
    def _ks_cr_menu_action(self, rec):
        ...
        if rec.ks_is_enable_multi_company:
            ...
            if allowed_company_ids and isinstance(allowed_company_ids[0], dict):
                allowed_company_ids = [c.get('id') for c in allowed_company_ids if 'id' in c]
            domain = [('x_company_id', 'in', allowed_company_ids)]
    ...
    def _ks_create_db_table(self):
        ...
        if not table:
            if self.ks_cr_query_type == 'custom_query':
                ...
                table = "x_ks_cr_ks_" + "".join([x[0] for x in self.ks_model_id.model.split(".")]) + str(self.id)

        tools.drop_view_if_exists(self.env.cr, table)

        if self.ks_cr_query_type == 'field_chain':
            table_query = self._create_chain_query(self)
        else:
            table_query = self._create_custom_query(self)
        ...
        if not cr_model_name:
            self._ks_cr_model(self, table)

        if self.ks_cr_query_type == 'field_chain':
            cr_model_name = self.ks_cr_model_id.model
            existing_field_names = set(self.env[cr_model_name]._fields) if cr_model_name in self.env else set()
            for field in table_query[1]:
                field_name = field[2]
                if field_name not in existing_field_names:
                    self._ks_cr_fields(field[0], field[1], field[2], field[3], field[4])
            self.ks_create_tree_view(self, table_query[2])
            self.ks_create_search_filter_view(self, table_query[3], table_query[4])
            if self.ks_is_enable_multi_company and self.ks_model_id.field_id.filtered(
                    lambda x: x.name in ('company_id', 'company_ids')):
                self._ks_create_ir_rule(self)
            if not self.ks_is_enable_multi_company:
                self.ks_unlink_ir_rule(self)
        else:
            self.ks_create_custom_fields(self, table_query[1])
            self.ks_create_tree_view(self, table_query[2])

        if self.ks_cr_model_id:
            self.env['ir.model.fields']._invalidate_cache()
            self.env.registry._setup_models__(self.env.cr)
            self.env['ir.ui.view']._invalidate_cache()
            self.env['ir.actions.act_window']._invalidate_cache()
    ...
    def ks_create_tree_view(self, rec, field_list):
        cr_model = rec.ks_cr_model_id
        safe_name = escape(rec.name + ' List View')
        rec.ks_tree_view_id = self.env['ir.ui.view'].sudo().create({
            'name': safe_name,
            'model': cr_model.model,
            'arch': '<list string="%s">' % safe_name +
                    "\n".join(['<field name="%s"/>' % x for x in field_list]) +
                    '</list>',
        })
    ...
    def ks_group_filter_arch(self, fields_desc, search_filter_field_list, group_filter_field_list, name, model):
        ...
        safe_name = escape('Search ' + name)
        values = {
            'name': name + ' Search View',
            'model': model,
            'arch': '<search string="%s">' % safe_name + search_filter_arch + group_arch + '</search>'
        }
        return values
    ...
    def ks_query_validate(self, ks_query, **kwargs):
        try:
            ...
            if kwargs.get('custom_user_error'):
                ...
                raise ValidationError(_("An error occurred while creating the view: %s") % str(kwargs.get('custom_user_error')))
            else:
                raise ValidationError(_("An error occurred while creating the view: %s") % str(e))
        finally:
            new_cr.close()
    ...
    def ks_get_company_wise_data(self,companyId):
        try:
            ...
            for rec in all_records:
                if rec.ks_is_enable_multi_company:
                    self.env.cr.execute("select name from ir_model_fields where model_id = %s",
                                        (rec.ks_cr_model_id.id,))
                    field_list = [x[0] for x in self.env.cr.fetchall()]
                    if companyId and isinstance(companyId[0], dict):
                        companyId = [c.get('id') for c in companyId if 'id' in c]
                    if 'x_company_id' in field_list:
                        rec.ks_cr_action_id.domain = [('x_company_id', 'in', companyId)]
                    else:
                        rec.ks_cr_action_id.domain = []
        except:
            pass
